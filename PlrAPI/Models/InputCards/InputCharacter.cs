    ...
    public class InputCharacter
    {
        ...
        public Character ToCharacter(Func<List<SocialFormation>> socFormsfill, Func<List<CharAdditionalValue>> additionalFieldsFill)
        {
            Character character = new Character
            {
                ...
                LocBirthId = this.LocBirthId == -1 ? null : this.LocBirthId,
                LocDeathId = this.LocDeathId == -1 ? null : this.LocDeathId,
                RaceId = this.RaceId,
                SocFormsId = this.SocFormsIds is not null ? new List<int>(this.SocFormsIds) : new List<int>(),
                Growth = this.Growth == -1 ? null : this.Growth,
                BioFatherId = this.BioFatherId == -1 ? null : this.BioFatherId,
                BioMotherId = this.BioMotherId == -1 ? null : this.BioMotherId,
                ...
                ColorHair = this.ColorHair is null || this.ColorHair.Length == 0 ? null : this.ColorHair,
                ColorEyes = this.ColorEyes is null || this.ColorEyes.Length == 0 ? null : this.ColorEyes,
                Desc = this.Desc,
